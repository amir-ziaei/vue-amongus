    ...
    <div class="container">
        <form @submit.prevent="submit">
            ...
            <div class="form-group">
                ...
                <button class="submit delete" type="submit">Delete Account</button>
            </div>
        </form>
      </div>
  </div>
...
export default {
  ...
  methods: {
    ...mapActions(["Update"]),
    async submit(e) {
      if(e.submitter.classList.contains('delete')) {
        this.deleteAccount();
        return;
      }
      this.$v.$touch()
      if (this.$v.$invalid) {
          ...
          .catch(err => {
            this.$notify({
              group: 'form-alerts',
              type: 'error',
              text: typeof err === 'object' ? err[Object.keys(err)[0]] : err,
              duration: 2000,
            });
          })
      }
...
.submit:hover {
  background-color: rgba(65, 180, 130, 0.9);
}
